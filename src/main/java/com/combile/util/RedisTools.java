...
public class RedisTools {
    ...
    private static final Integer SLEEPTIME = 50;


    //=============================common============================
    public void pushList(String key, List<String> list) {
        Objects.requireNonNull(key, "redis list操作key不能为空");
    ...
    public List<String> getList(String key) {
        ...
        return this.stringRedisTemplate.opsForList().range(key, 0, -1);
    }

    public void hashSet(String key, String field, String value) {
        Objects.requireNonNull(key, "redis hashSet操作key不能为空");
        Objects.requireNonNull(field, "redis hashSet操作key不能为空");
        this.stringRedisTemplate.opsForHash().put(key, field, value);
    }

    public String hashGet(String key, String field) {
        Objects.requireNonNull(key, "redis hashGet操作key不能为空");
        Objects.requireNonNull(field, "redis hashGet操作key不能为空");
        Object val = this.stringRedisTemplate.opsForHash().get(key, field);
    ...
    public Boolean getLock(String lockKey) {
        ...
        SessionCallback<Boolean> sessionCallback = new SessionCallback<Boolean>() {
            ...
            public Boolean execute(RedisOperations operations) throws DataAccessException {
                operations.multi();
                stringRedisTemplate.opsForValue().setIfAbsent(lockKey, "1");
                stringRedisTemplate.expire(lockKey, 600, TimeUnit.SECONDS);
     ...
     * @param lockKey
     */
    public Boolean getLock(String lockKey, Integer seconds) {
        log.info("lockKey : {}", lockKey);
        // 使用sessionCallBack处理
        SessionCallback<Boolean> sessionCallback = new SessionCallback<Boolean>() {
            List<Object> exec = null;
            ...
            public Boolean execute(RedisOperations operations) throws DataAccessException {
                operations.multi();
                stringRedisTemplate.opsForValue().setIfAbsent(lockKey, "1");
                stringRedisTemplate.expire(lockKey, seconds, TimeUnit.SECONDS);
                exec = operations.exec();
                if (exec.size() > 0) {
                    return (Boolean) exec.get(0);
                }
    ...
    public Boolean getLockRoll(String lockKey, Integer seconds) {

        Boolean lock = false;

        do {
            if (getLock(lockKey, seconds)) {
                return true;
            } else {
                try {
                    Thread.sleep(50);
